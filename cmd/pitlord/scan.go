...
func runScanWithLoader(args []string, stdout, stderr io.Writer, loader scan.GraphLoader) int {
	...
	guardAnalyzerSpec := flags.String("guard-analyzers", "", "selected analyzer IDs whose findings should use guard disposition")
	...
	result, err := (scan.Engine{Loader: loader, Analyzers: analyzers}).Run(ctx, scan.Input{
		...
		GuardAnalyzers: splitCommaList(*guardAnalyzerSpec),
	})
	if err != nil {
		fmt.Fprintln(stderr, err)
		return 2
