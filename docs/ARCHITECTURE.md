...
pinned corpus + calibration reference
  -> Git revision and optional worktree-diff verification
  -> exact built-in analyzer selection
  -> optional rule/language/source-location filtering
  -> labelled evaluation and optional fully-labelled regression gate
...
- calibration-reference validation and deterministic scoring of exact built-in detector/analyzer output against frozen corpus ground truth, including optional rule, language, source-location, and fully-labelled selectors;
...
`calibrate` adds a reference-evaluation layer around the same scan engine. A reference pins a Git source revision and exactly one execution target: legacy architecture `detector` or built-in `analyzer` ID. Analyzer references may additionally constrain `rule_id` and `language`; exact-path expectations may constrain source location; controlled mutation specimens may pin the SHA-256 of the complete tracked worktree diff. Revision or worktree-diff mismatch fails closed before evaluation. The command executes only the selected analyzer, resolves Arcana only when that analyzer requires graph state, and passes the repository root through for graph-free analyzers such as Clippy. Required and absent labels contribute to labelled precision/recall, allowed labels can constrain severity, and unmatched target findings remain explicitly unlabelled. References with `require_fully_labelled` promote any such unlabelled target finding to mismatch status for deterministic regression gating without changing legacy exploratory calibration semantics.
