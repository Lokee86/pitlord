...
the same game-server source root. That mutation is a symbol-level intermediary bypass inside one source file, so it is intentionally outside the current file/region `boundary-bypass` detector; a future symbol-level detector must own that case without inferring symbol shortcuts from file topology.
