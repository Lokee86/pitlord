...
`pitlord scan` currently implements three generalized advisory detectors: calibrated language-neutral outgoing cross-file `dependency-pressure`, calibrated language-neutral `dependency-knots` over strongly connected static source-dependency components, and `boundary-cohesion` over semantic architectural regions. Blast-radius/hotspot analysis remains to be implemented. All three detectors have deterministic synthetic topology coverage and frozen detector-specific projections across the same 16 real-world corpora.
