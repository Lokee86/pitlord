...
The impact/blast-radius detector is deliberately conservative. The untouched transitive-closure rule emitted 74 findings and scored 0 TP / 7 TN / 74 FP / 0 FN. Calibration showed that catalogs, stable contracts, base types, annotations, exceptions, and shared utilities can inherit one gateway's downstream closure without independently owning that change surface. The tuned detector therefore requires at least three substantial independent first-hop branches, where each branch contributes at least 5% of total reach (minimum two exclusive files), and rejects candidates whose largest branch exceeds 80% of total reach. The frozen projection is now 0 TP / 81 TN / 0 FP / 0 FN with no severity or unlabelled mismatches. There is no required real-world positive yet, so this validates false-positive control rather than real-world recall; the detector remains advisory.

**Removal condition:** boundary/cohesion and impact/blast-radius gain required real-world positive coverage, and each generalized detector has sufficient recall evidence to support its intended advisory or guard disposition without weakening the current false-positive controls.
