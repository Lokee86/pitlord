...
`pitlord scan` currently implements four generalized advisory detectors: calibrated language-neutral outgoing cross-file `dependency-pressure`, calibrated language-neutral `dependency-knots` over strongly connected static source-dependency components, `boundary-cohesion` over semantic architectural regions, and calibrated language-neutral `hub-bottleneck` analysis over many-to-many behavioral coordination waists. Impact/blast-radius hotspot analysis remains to be implemented. All four detectors have deterministic synthetic topology coverage and frozen detector-specific projections across the same 16 real-world corpora.
