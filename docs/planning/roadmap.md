...
1. Add required real-world positive calibration coverage for boundary/cohesion and impact/blast-radius before making recall or guard-readiness claims for those detector families.
...
- The fifth generalized detector, `impact-blast-radius`, is implemented as an advisory file-level transitive-impact judgment. Broad reach is reported only when at least three first-hop dependent branches each contribute a substantial exclusive downstream set and no one branch contributes more than 80% of the closure, preventing one gateway's reach from being inherited by every leaf beneath it. Against 81 frozen real-corpus negative controls the tuned detector has 0 false positives and 0 unlabelled findings; synthetic independent-branch topology supplies the positive control until a required real-world positive is added.
