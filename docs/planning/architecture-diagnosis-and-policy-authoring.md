...
4. **Impact and blast-radius hotspots** — files whose transitive dependents cover an unusually large portion of the repository or subsystem through several substantial independent first-hop branches, rather than merely inheriting one gateway's downstream closure.
...
Impact/blast-radius calibration established that raw transitive closure is likewise insufficient. The untouched detector emitted 74 findings, including whole Spectre.Console border families, Maven annotations/contracts, JMH utilities, HikariCP timing/metrics contracts, and other foundational leaves whose apparent reach was inherited through downstream gateways. The tuned detector requires at least three substantial independent first-hop dependent branches; each branch must contribute an exclusive downstream set of at least 5% of total reach, with a two-file minimum, and the largest branch may contribute at most 80% of total reach. Healthy layering and single-gateway inheritance are negative controls, while balanced independently expanding branches remain the synthetic positive. Against 81 frozen real-corpus negative controls, the tuned projection is 0 TP / 81 TN / 0 FP / 0 FN with no severity or unlabelled mismatches. Because the projection contains no required real-world positive, impact/blast-radius remains advisory and real-world recall is not claimed.
