...
## Status
In progress. The policy-free `scan` command, deterministic `pitlord.scan.v1` finding envelope, and nine generalized detector families are implemented: `dependency-pressure`, `dependency-knots`, `dependency-depth`, `unstable-dependency-direction`, `boundary-bypass`, `symbol-intermediary-bypass`, `boundary-cohesion`, `hub-bottleneck`, and `impact-blast-radius`. Dependency knots, dependency depth, hub/bottleneck, impact/blast-radius, and symbol intermediary bypass have required real-repository positives in their frozen projections. Dependency depth scores 1 TP / 16 TN / 0 FP / 0 FN against its independently audited 17-expectation projection; impact/blast-radius scores 19 TP / 52 TN / 0 FP / 0 FN against its corrected 150-expectation projection. Symbol intermediary bypass preserves 16 whole-repository negative controls and detects one diff-pinned Space Rocks Homunculus development mutation, but validation/holdout recall remains untested because that development specimen shaped the detector. Unstable dependency direction and boundary bypass each score 0 TP / 16 TN / 0 FP / 0 FN against independently audited false-positive projections, and boundary/cohesion has likewise completed false-positive tuning without a required real-world positive; recall remains unvalidated for those three families. The disposition workflow and policy-authoring flow remain planned. This document otherwise describes target behavior rather than shipped detector behavior.
...
2. **Hub and bottleneck candidates** — nodes, files, packages, or communities where central reuse converges with broad coordination responsibility. The shipped file-level `hub-bottleneck` detector uses extreme direct fan-in only as candidate generation and requires a many-to-many behavioral waist over `calls`, `reads`, and `writes`; package/community aggregation remains follow-on work.
...
4. **Impact and blast-radius hotspots** — files with unusually broad direct dependent exposure across architectural regions, or whose lower-direct-fan-in transitive dependents cover an unusually large portion of the repository through several substantial independent first-hop branches rather than merely inheriting one gateway's downstream closure.
5. **Dependency depth corridors** — narrow acyclic behavioral chains that cross successive architectural regions, with cycles, import-only paths, shared convergence, and reused central foundations excluded from inherited depth.
6. **Boundary bypasses** — exceptional file-level behavioral shortcuts around an intermediary established by peer flow, with shared targets, weak gateway concentration, nested ownership, and import/reference-only reach excluded.
7. **Unstable dependency direction** — acyclic static source dependencies from an incoming-heavy architectural region toward an outgoing-heavy region across a material instability gradient, with multi-file boundary support and region cycles excluded.

Likely follow-on detectors include symbol-level intermediary bypasses, dead or orphaned regions, graph-community/filesystem mismatches, architectural erosion across snapshots, and change-aware anomalies when source-history evidence is available.
...
The completed 16-corpus calibration pass replaced that initial coarse behavior with a boundary-aware production-peer model. Common test, benchmark, sample, generated, vendor, tooling, and devtool paths no longer enter the production peer population. Boundary-aware candidates must remain raw fan-out outliers, rank in the top 5% of active production peers, cross at least three target directories, and have at least 35% cross-directory spread. Conventional composition/entrypoint/controller/invoker/factory seams are deferred, compatibility paths do not escalate above warning, and strongly reused central hubs are left to `hub-bottleneck`. Against the frozen detector-specific references, the current projection scores 3 true positives and 105 true negatives with zero labelled false positives, false negatives, or severity mismatches. Twenty-six findings remain explicitly unlabelled rather than being counted as successes, so dependency pressure remains advisory while those cases and the remaining detector families are completed.

Boundary/cohesion calibration exposed a different failure mode: Tukey outlier fences over bounded `[0,1]` ratios became mathematically unreachable on real corpora. The tuned detector instead evaluates semantic namespace or multi-file-module regions, uses static dependency-like edges for outward boundary spread, and uses a broader relation set including calls and reads/writes for internal collaboration. A candidate needs at least six files and six outgoing cross-region dependencies, at least three independent target regions, at least 0.25 target regions per member, top-decile target-region reach, internal support in the weakest peer quartile, and no more than 0.5 internal support relationships per member. Incoming-heavy regions are deferred to the bottleneck family. All 30 frozen real-corpus negative controls remain quiet with no unlabelled findings, while the synthetic weak/outward fixture remains positive. Because the frozen real-corpus projection contains no required positive, this is false-positive calibration rather than real-world recall validation.

Hub/bottleneck calibration established that direct centrality is also insufficient. The untouched detector required extreme direct fan-in, multiple incoming regions, and non-trivial fan-out, yet emitted 37 findings and scored 1 TP / 10 TN / 33 FP / 0 FN. Maven public/session/artifact models, JMH result models, jsoup DOM roots, and other shared abstractions looked two-sided only because signature/reference edges inflated their apparent responsibility. The tuned detector therefore separates candidate generation from judgment: extreme direct fan-in remains the entry condition, but only `calls`, `reads`, and `writes` count as the decisive behavioral flow. A finding requires at least six incoming behavioral regions, at least four outgoing behavioral regions, and behavioral outgoing degree at least 40% of behavioral incoming degree. That preserves the required Space Rocks networking gravity well while making all 43 frozen absent controls quiet; the tuned projection is 1 TP / 43 TN / 0 FP / 0 FN with no severity or unlabelled mismatches. The synthetic positive is now a true many-to-many behavioral waist, while the old reference-heavy hub fixtures are negative controls.

Impact/blast-radius calibration established two separate failures in the original approach. Raw transitive closure emitted 74 findings dominated by inherited gateway/catalog reach, while the first branch-gated tuning removed that noise by suppressing every required real-world positive. An independent 16-corpus audit corrected the ground truth and established two owned shapes: broad direct exposure and lower-direct-fan-in transitive amplification. The direct lane requires repository-relative extreme fan-in, cross-region breadth, and non-metadata dependency evidence; imports, generic references, and annotations do not establish that evidence alone. The transitive lane retains the independent first-hop branch discriminator: at least three substantial branches, each with an exclusive downstream set of at least 5% of reach and a two-file minimum, with no branch above 80% of total closure. Against 150 frozen expectations the current two-lane detector scores 19 TP / 52 TN / 0 FP / 0 FN with no severity mismatches or unlabelled findings. It remains advisory because broad impact can be intentional, not because recall is unvalidated.

Dependency-depth calibration similarly separated reachable graph length from architectural depth. An apparent Maven four-hop path depended on an import-only middle edge and disappeared when imports and generic references were removed from depth propagation. The independent audit then left one source-validated JMH corridor spanning four hops, four architectural regions, and three region transitions. Spectre.Console also contains a four-hop behavioral path, but it remains within two regions and is a frozen negative control. The detector stops at cycles, shared convergence points, and highly reused central foundations so downstream implementation depth is not inherited by unrelated callers. Across 17 frozen expectations the final projection is 1 TP / 16 TN / 0 FP / 0 FN with no severity mismatches or unlabelled findings.

Unstable-dependency-direction calibration tested a region-level Stable Dependencies Principle rather than treating every positive instability gradient as suspicious. The raw detector emitted zero findings across all 16 pinned corpora, so the audit enumerated suppressed candidates instead of accepting the zero-output result. Polly, Gson, HikariCP, jsoup, kotlinx.coroutines, and Lexicanter had stable-to-unstable edges only inside region SCCs, validating deferral to dependency-knot ownership. After cycle, region-size, coupling, and multi-source gates, only JMH generator core -> runner (`I 0.444 -> 0.625`, gap `0.181`) and Maven compatibility model-building -> model-io (`I 0.425 -> 0.667`, gap `0.242`) remained. Source audit showed the JMH edge is a deliberate generator/runtime resource contract and the Maven edge is explicitly deprecated compatibility architecture. Relaxing the cross-half requirement additionally exposed healthy adapters and shared infrastructure. No supported acyclic positive-gradient boundary reached the current `0.25` gap. The final projection is therefore 0 TP / 16 TN / 0 FP / 0 FN with no severity or unlabelled mismatches, explicitly validating precision controls rather than recall.

Boundary-bypass calibration established that a dependency triangle does not imply an architectural gateway. The initial peer→gateway→target rule emitted 18 false gateway inferences across Gson and Space Rocks. Source audit added three discriminators: the gateway must be materially concentrated on the downstream region, the target cannot already be broadly shared across regions, and nested source/target ownership trees do not establish a sibling intermediary requirement. A relaxed near-miss audit then examined JMH, Maven, jsoup, Lexicanter, Space Rocks, and other candidate families without finding a defensible file/region positive. The final 16-control projection is 0 TP / 16 TN / 0 FP / 0 FN with no severity or unlabelled mismatches, so it is false-positive calibration rather than recall validation. The prior Space Rocks debug-score mutation remains a separate symbol-level bypass because the skipped intermediary and caller occupy the same file.

## Initial implementation direction
1. Build on the implemented policy-free `scan` seam and completed nine-family generalized detector set by finalizing stable evidence, advisory-versus-guard disposition, and mechanically verifiable required outcomes for agent-facing workflows.
