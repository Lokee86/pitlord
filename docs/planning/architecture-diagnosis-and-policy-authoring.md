...
## Status
In progress. The policy-free `scan` command, deterministic `pitlord.scan.v1` finding envelope, and six generalized detector families are implemented: `dependency-pressure`, `dependency-knots`, `dependency-depth`, `boundary-cohesion`, `hub-bottleneck`, and `impact-blast-radius`. Dependency knots, dependency depth, hub/bottleneck, and impact/blast-radius have required real-world positives in their frozen projections. Dependency depth scores 1 TP / 16 TN / 0 FP / 0 FN against its independently audited 17-expectation projection; impact/blast-radius scores 19 TP / 52 TN / 0 FP / 0 FN against its corrected 150-expectation projection. Boundary/cohesion has completed false-positive tuning against frozen real-corpus controls but still lacks a required real-world positive for recall validation. The disposition workflow and policy-authoring flow remain planned. This document otherwise describes target behavior rather than shipped detector behavior.
...
Likely follow-on detectors include intermediary bypasses, dead or orphaned regions, graph-community/filesystem mismatches, architectural erosion across snapshots, unstable dependency direction, and change-aware anomalies when source-history evidence is available.
...
1. Build on the implemented policy-free `scan` seam and completed six-family generalized detector set by finalizing stable evidence, advisory-versus-guard disposition, and mechanically verifiable required outcomes for agent-facing workflows.
