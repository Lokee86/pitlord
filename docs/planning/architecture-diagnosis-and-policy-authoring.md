...
## Status
In progress. The policy-free `scan` command, deterministic `pitlord.scan.v1` finding envelope, calibrated language-neutral `dependency-pressure` detector, calibrated language-neutral `dependency-knots` detector, advisory `boundary-cohesion` detector, and calibrated advisory `hub-bottleneck` detector are implemented. The knot detector has completed tuning against its frozen 16-corpus detector-specific baseline; boundary/cohesion has completed tuning against 30 frozen negative controls but still lacks a required real-world positive for recall validation; hub/bottleneck has completed tuning against its frozen 16-corpus projection with 1 TP / 43 TN / 0 FP / 0 FN and no severity or unlabelled mismatches. Impact/blast-radius analysis, the disposition workflow, and the policy-authoring flow remain planned. This document otherwise describes target behavior rather than shipped detector behavior.
