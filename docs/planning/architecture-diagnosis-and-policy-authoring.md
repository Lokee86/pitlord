...
A broader five-control run on August 27, 2026 established the next calibration requirements. Dapper (C#) produced 12 advisories (5 high, 7 warning), jsoup (Java) 18 warnings, detekt (Kotlin) 8 warnings, Lexicanter (Svelte/Rust) 3 advisories (1 high, 2 warning), and the frozen Space Rocks polyglot clean control 20 high advisories. The run confirms that the detector is adapter/language-neutral in practice, but the clean-control and fine-grained-repository results show that whole-repository median comparison and current severity thresholds are too aggressive for blocking use. Test/benchmark sources also need distinct peer treatment, and remediation must distinguish genuine responsibility sprawl from legitimate entrypoint, coordinator, or core-domain breadth. These issues should be corrected and the same controls rerun before promoting dependency pressure from advisory use or treating its current severity scale as stable.
