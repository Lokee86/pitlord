...
## Status
In progress. The policy-free `scan` command, deterministic `pitlord.scan.v1` finding envelope, and all five initial generalized detector families are implemented: `dependency-pressure`, `dependency-knots`, `boundary-cohesion`, `hub-bottleneck`, and `impact-blast-radius`. Dependency knots, hub/bottleneck, and impact/blast-radius have required real-world positives in their frozen projections. Impact/blast-radius now scores 19 TP / 52 TN / 0 FP / 0 FN against its corrected 150-expectation projection. Boundary/cohesion has completed false-positive tuning against frozen real-corpus controls but still lacks a required real-world positive for recall validation. The disposition workflow and policy-authoring flow remain planned. This document otherwise describes target behavior rather than shipped detector behavior.
...
1. Build on the implemented policy-free `scan` seam and completed five-family initial detector set by finalizing stable evidence, advisory-versus-guard disposition, and mechanically verifiable required outcomes for agent-facing workflows.
