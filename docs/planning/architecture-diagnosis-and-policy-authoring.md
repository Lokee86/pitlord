...
## Status
In progress. The policy-free `scan` command, deterministic `pitlord.scan.v1` finding envelope, calibrated language-neutral `dependency-pressure` detector, and calibrated language-neutral `dependency-knots` detector are implemented. The knot detector has completed tuning against its frozen 16-corpus detector-specific baseline; the remaining generalized detector families, disposition workflow, and policy-authoring flow remain planned. This document otherwise describes target behavior rather than shipped detector behavior.
