...
## Status
In progress. The policy-free `scan` command, deterministic `pitlord.scan.v1` finding envelope, and eight generalized detector families are implemented: `dependency-pressure`, `dependency-knots`, `dependency-depth`, `unstable-dependency-direction`, `boundary-bypass`, `boundary-cohesion`, `hub-bottleneck`, and `impact-blast-radius`. Dependency knots, dependency depth, hub/bottleneck, and impact/blast-radius have required real-world positives in their frozen projections. Dependency depth scores 1 TP / 16 TN / 0 FP / 0 FN against its independently audited 17-expectation projection; impact/blast-radius scores 19 TP / 52 TN / 0 FP / 0 FN against its corrected 150-expectation projection. Unstable dependency direction and boundary bypass each score 0 TP / 16 TN / 0 FP / 0 FN against independently audited false-positive projections, and boundary/cohesion has likewise completed false-positive tuning without a required real-world positive; recall remains unvalidated for all three families. The disposition workflow and policy-authoring flow remain planned. This document otherwise describes target behavior rather than shipped detector behavior.
...
1. Build on the implemented policy-free `scan` seam and completed eight-family generalized detector set by finalizing stable evidence, advisory-versus-guard disposition, and mechanically verifiable required outcomes for agent-facing workflows.
