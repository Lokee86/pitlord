...
| Generalized scans require no Pitlord policy, preserve deterministic finding ordering plus non-null finding/evidence arrays, allow explicit promotion only from named selected analyzer IDs to guard disposition, fail closed on invalid guard selection, and return exit 1 exactly when the finalized result contains guard findings | `internal/scan/scan_test.go`, `cmd/pitlord/scan_test.go`, `internal/report/scan_test.go` |
