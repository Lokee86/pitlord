...
| Impact/blast-radius detection requires top-5% transitive reach over >=25% of production peers, >=2x amplification over direct fan-in, >=3 impacted regions, >=3 substantial independent first-hop dependent branches, and <=80% dominant-branch share; inherited single-gateway closure, pure direct fan-in, dominant-branch reach, ordinary layering, and family-wide reachability remain quiet | `internal/scan/impact_blast_radius_test.go`, `internal/scan/impact_blast_radius.go`, `internal/scan/impact_blast_radius_metrics.go` |
