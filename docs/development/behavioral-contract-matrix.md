...
| Generalized scans require no Pitlord policy, preserve deterministic finding ordering plus non-null finding/evidence arrays, and return exit 1 exactly when the finalized result contains guard-disposition findings | `internal/scan/scan_test.go`, `cmd/pitlord/scan_test.go`, `internal/report/scan_test.go` |
