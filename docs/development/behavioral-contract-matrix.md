...
| Dependency-knot detection uses production file nodes, directional normalized relations rather than generic references/read/write edges, deterministic iterative SCC traversal, cross-directory significance in repository scopes, density fallback in single-directory scopes, and stable component-level finding identity | `internal/scan/dependency_knots_test.go`, `internal/scan/dependency_knots_calibration_test.go`, `internal/scan/dependency_scc.go` |
