...
- a graph-backed `semantic` analyzer group whose rules declare language-neutral capabilities supplied by Lexicon adapters; the first rule, `swallowed-error`, consumes normalized Rust, TypeScript, and Python error-handler facts and reports handlers that neither propagate, record, nor explicitly recover from an error;
