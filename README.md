...
- a graph-backed `semantic` analyzer group whose rules declare language-neutral capabilities supplied by Lexicon adapters; the first rule, `swallowed-error`, consumes normalized Rust and TypeScript error-handler facts and reports handlers that neither propagate, record, nor explicitly recover from an error;
